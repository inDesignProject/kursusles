<?php
	...
	if( $enkripsi->decode($_GET['func']) == "saveWith" && isset($_GET['func'])){
		...
		if($affected > 0){
			...
			$sqlSel		=	sprintf("SELECT B.NAMA, B.EMAIL, A.NOMINAL, B.IDPENGAJAR, C.NO_REKENING, C.ATAS_NAMA, D.NAMA_BANK
									 FROM t_withdraw A
			...
			$idpengajar	=	$resultsel['IDPENGAJAR'];
			...
			$sqlupdB	=	sprintf("UPDATE m_pengajar A
									 LEFT JOIN t_withdraw B ON A.IDPENGAJAR = B.IDUSER
									 SET A.CURRENT_BALANCE = A.CURRENT_BALANCE - %s
			...
			$message	=  "<html>
							...
							<body>
								...
								Balance anda akan secara otomatis berkurang sejumlah nominal diatas.
								<br/><br/>
	...
	if( $enkripsi->decode($_GET['func']) == "appWith" && isset($_GET['func'])){
		...
		$sql	=	sprintf("SELECT NAMA, CURRENT_BALANCE
							 FROM t_withdraw A
							 LEFT JOIN m_pengajar B ON A.IDUSER = B.IDPENGAJAR
		...
		echo "	<tr id='detail".$_POST['id']."' class='detail-con'>
					<td colspan='8' style='padding: 18px'>
						<h4>Data Pengajar</h4><br>
						<i class='fa fa-lg fa-user'></i> Nama : ".$result['NAMA']."<br>
						<i class='fa fa-lg fa-tag'></i> Balance : Rp. ".number_format($result['CURRENT_BALANCE'],0,',','.').",-<br><br>
	...
	if( $enkripsi->decode($_GET['func']) == "filterData" && isset($_GET['func'])){
		...
		//SELECT DATA
		$sql		=	sprintf("SELECT A.TANGGAL, B.NAMA, B.IDPENGAJAR, C.NO_REKENING AS REKENING_ASAL, C.ATAS_NAMA AS AN_ASAL,
								 ...
								 WHERE A.TANGGAL BETWEEN '%s' AND '%s' AND %s AND A.TYPE = 1"
								, $_POST['tglfrom']
								, $_POST['tglto']
								, $conStatus
								);
		...
		if($result <> '' && $result <> false){
			...
			foreach($result as $key){
				...
				$data	.=	"	<tr id='baris".$enkripsi->encode($key['IDWITHDRAW'])."'>
									...
									<td><a target='_blank' href='".KURSUSLES_URL."pengajar_profil?q=".$enkripsi->encode($key['IDPENGAJAR'])."'>".$key['NAMA']."</a></td>
									<td>
										".$rek_asal."
									</td>
									<td>
...
<div class="pageheader notab">
    ...
    <br clear="all" />
    <h1 class="pagetitle">Withdraw Dana</h1>
</div>
            	...
            	<table id="standart_table" class="stdtable" border="0" cellspacing="0" style="margin: 0 4px;width: 99%;">
                    <thead>
                        <tr align="center" style="font-weight:bold;">
                            ...
                            <td class="head1" style="text-align:center;">Nama Pengajar</td>
...
<script>
	...
	function procIgnoreWith(id){
		...
		$.post( "<?=APP_URL?>page/300withdraw.php?func=<?=$enkripsi->encode('procIgnoreWith')?>", data)
		.done(function( data ) {
	...
	function saveWith(id){
		...
		$('#message_response_container').slideDown('fast').html(generateMsg("Sedang menyimpan"));
		$.post( "<?=APP_URL?>page/300withdraw.php?func=<?=$enkripsi->encode('saveWith')?>", data)
		.done(function( data ) {
			data	=	JSON.parse(data);
			if(data['respon_code'] == '00000'){
				filterData(1);
	...
	function setWith(id, status){
		...
		if(status == true){
			$.post( "<?=APP_URL?>page/300withdraw.php?func=<?=$enkripsi->encode('appWith')?>", {id:id})
			.done(function( data ) {
				$('#baris'+id).after(data);
				getDataOpt('getDataRekeningAdmin','noparam','rekeningadmin','','- Pilih Rekening Asal -');
			});
		} else {
			$.post( "<?=APP_URL?>page/300withdraw.php?func=<?=$enkripsi->encode('ignWith')?>", {id:id})
			.done(function( data ) {
				$('#baris'+id).after(data);
			});
		}
	...
	function filterData(page){
		...
		$.post( "<?=APP_URL?>page/300withdraw.php?func=<?=$enkripsi->encode('filterData')?>", {tglto: tglto, tglfrom:tglfrom, status:status, page:page})
		.done(function( data ) {
