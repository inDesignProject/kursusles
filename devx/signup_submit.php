...
if( $enkripsi->decode($_GET['func']) == "submitForm" && isset($_GET['func'])){
	...
	$sqlUser	=	sprintf("INSERT IGNORE INTO m_user (USERNAME,PASSWORD,IDLEVEL,TGL_DAFTAR) VALUES ('%s',MD5('%s'),%s,NOW())"
							, $username
							, $password1
							, $idlevel
							);
	...
	if($lastIDUser > 0){
		...
		$sqlins		=	sprintf("INSERT INTO %s (NAMA,ALAMAT,JNS_KELAMIN,TELPON,TGL_LAHIR,IDKOTA_LAHIR,IDKOTA_TINGGAL,IDKECAMATAN,IDKELURAHAN,KODEPOS,EMAIL)
									VALUES
		...
		if($lastID > 0){
			...
			$sqlUpdate	=	sprintf("UPDATE m_user SET IDUSER_CHILD = %s WHERE IDUSER = %s"
									, $lastID
									, $lastIDUser
									);
