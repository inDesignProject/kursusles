<?php
	...
	session_start();
	$sql		=	sprintf("SELECT KTP FROM m_pengajar WHERE IDPENGAJAR = %s", $_SESSION['KursusLes']['IDUSER']);
	$result		=	$db->query($sql);
	$foto		=	$result[0]['KTP'];
	...
	if( $enkripsi->decode($_GET['func']) == "setFoto" && isset($_GET['func'])){
		...
		if(file_exists('../../images/ktp/'.$filename)){
			rename('../../images/ktp/'.$filename, '../../images/ktp/rn_'.date('YmdHis').$filename);
		}
		
		//RENAME TEMPORARY FILE
		if(file_exists('../../images/ktp/tmp_'.$filename)){
			
			$sqlUpdate	=	sprintf("UPDATE m_pengajar
									 SET KTP			=	'%s', VERIFIED_STATUS = -1
									 WHERE IDPENGAJAR	=	%s"
									, $filename
			...
			if(rename('../../images/ktp/tmp_'.$filename, '../../images/ktp/'.$filename)){
...
<head>
	...
	<?php
	if($foto <> "" && $foto <> 'null'){
		$style_foto		=	"background: url('".APP_IMG_URL."generate_pic.php?type=ktp&q=".$enkripsi->encode($foto)."') no-repeat scroll -8px top / cover transparent;";
	}
	?>
	</style>
</head>
<body>
	<div id="upload_container" style="width: 445px !important;">
        <div id="userpic" class="userpic">
            <div class="js-preview userpic__preview" style="<?=$style_foto?> width: 407px;height: 271px;"></div>
    ...
    <div id="editor_status" class="info">
        ...
        <p id="editor_message">
        	<?=$foto == '' || $foto == 'null' ? "Anda belum memiliki scan identitas. Silakan pilih dan upload dengan klik tombol <b>Pilih File</b>" : "Silakan pilih file yang akan diunggah"?>
        </p>
    </div>
	...
	<script>
		...
		examples.push(function (){
			var response;
			$('#userpic').fileapi({
				url: '../lib/upload_ktp_ctrl.php',
				accept: 'image/jpg,image/jpeg',
				imageSize: { minWidth: 500, minHeight: 300 },
				elements: {
