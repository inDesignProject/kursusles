...
//		$idpengajar	=	$_GET['q'] == "" && !isset($_GET['q']) ? $_SESSION['KursusLes']['IDUSER'] : $enkripsi->decode($_GET['q']) ;
		...
		$sqlDetail	=	sprintf("SELECT A.IDPENGAJAR, A.NAMA, CONCAT(A.ALAMAT, ' Kec. ', D.NAMA_KECAMATAN, ' - ', E.NAMA_KOTA) AS ALAMAT_LENGKAP,
		...
		if(isset($_SESSION['KursusLes']['IDUSER'])){
			$login_f	=	'false';
		} else {
			$login_f	=	'true';
		}
...
<html lang="id">
    ...
    <body>
						...
						<div class="boxSquareWhite">
							<div class="row">
                                ...
                                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
                                    ...
                                    if($_SESSION['KursusLes']['TYPEUSER'] == 2 && isset($_SESSION['KursusLes']['IDUSER']) && $_SESSION['KursusLes']['IDUSER'] == $idpengajar){
                                    ?>
										<button class="btn btn-custom btn-xs pull-right" onclick="window.location.href='editdata?q=<?=$enkripsi->encode($_SESSION['KursusLes']['IDUSER'])?>&r=pengajar_profil'"><i class="fa"></i>Edit Data Saya</button>
                                    <?php
                                    } else {
                                    ?>
        ...
        <script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141218'.'jsspengajarprofil');?>.jsfile&PARAM=<?=$enkripsi->encode($idpengajar)?>"></script>
