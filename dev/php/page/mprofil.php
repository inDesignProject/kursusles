...
session_start();
$idmurid	=	$_GET['q'] == "" && !isset($_GET['q']) ? $_SESSION['KursusLes']['IDPRIME'] : $enkripsi->decode($_GET['q']) ;
//$idmurid	=	$_SESSION['KursusLes']['IDPRIME'];
...
if( $enkripsi->decode($_GET['func']) == "resetPassword" && isset($_GET['func'])){
	...
	//CEK PASSWORD LAMA
	$sqlCek		=	sprintf("SELECT * FROM m_user A
...
if( $enkripsi->decode($_GET['func']) == "saveProfil" && isset($_GET['func'])){
	...
	$sqlUpd		=	sprintf("UPDATE m_murid SET TENTANG = '%s' WHERE iduser = %s"
	...
	if($affected > 0){
		...
		$sqlUpd		=	sprintf("INSERT INTO m_murid (TENTANG, iduser) VALUES ('%s', %s)"
							, $db->db_text($_POST['value'])
							, $_SESSION['KursusLes']['IDPRIME']
							);
		$affected	=	$db->execSQL($sqlUpd, 0);
