<?php
	...
	$xx = '';
	$sql = '';
	$sql .= 'select idpengajar from m_pengajar where iduser ='.$_SESSION['KursusLes']['IDPRIME'];
	
	$isPengajar	=	$db->query($sql);
	$idPengajar = $isPengajar[0]['idpengajar'];
	if(isset($_POST['id_pengajar'])){
	
	
		$sqll = '';
		$sqll .= "SELECT * FROM m_mapel WHERE nama_mapel ='".$_POST['nama_mapel']."' ";
		$resultl		=	$db->query($sqll);
		$resultl		=	$resultl[0];
		if($resultl == false){
			$sqly = '';
			$sqly .= "SELECT * FROM m_jenjang WHERE kode_jenjang ='".$_POST['jenjang_pendidikan']."' ";
			$resulty		=	$db->query($sqly);
			$resulty		=	$resulty[0];	

			$sqlx = '';
			$sqlx .= 'insert into m_mapel (idjenjang, nama_mapel,kode_mapel,status, id_pengajar) ';
			$sqlx .= 'VALUES (';
			$sqlx .= $resulty['IDJENJANG'].', ';
			$sqlx .= "'".strtoupper($_POST['nama_mapel'])."', ";
			$sqlx .= "'".strtoupper(substr($_POST['nama_mapel'],0,5))."', ";
			$sqlx .= '1, ';
			$sqlx .= $_POST['id_pengajar'].') '; 
	...
	$idpengajar	=	($_GET['q'] == "" && !isset($_GET['q'])) ? $idPengajar : $enkripsi->decode($_GET['q']) ;
	...
	$sql		=	sprintf("SELECT A.IDMAPEL, A.NAMA_MAPEL, B.KODE_JENJANG, B.NAMA_JENJANG, C.IDPENGAJAR,
							 ...
							 WHERE A.STATUS = 1 AND (A.id_pengajar = 0 OR A.id_pengajar  = ".$idpengajar.") 
		...
		if(!in_array($key['KODE_JENJANG'], $jenjang)){
			...
			if($i == 0){
				...
				// $tabcontent .= "<div class='tab-paket tab-paket2'>";
					//$tabcontent .= "<input name='listMapel_others' id='listMapel_others' onclick='addRemoveList(this.value,\"listMapel_others\")' type='checkbox' value='others' ".$checked."/>";
		//$tabcontent .= "<span> <b>Others</b></span></div><br>";
				...
				// $tabcontent .= "<div class='tab-paket tab-paket2'>";
				//	$tabcontent .= "<input name='listMapel_others' id='listMapel_others' onclick='addRemoveList(this.value,\"listMapel_others\")' type='checkbox' value='others' ".$checked."/>";
		//$tabcontent .= "<span> <b>Others</b><input type=text name='txt_others_".$key['KODE_JENJANG']."' id='txt_others_".$key['KODE_JENJANG']."' value='".$key['KODE_JENJANG']."'></span></div><br>";
			}
...
<html lang="id">
    ...
    <body>
        ...
        <div class='container'>
            <h3 class="text-left text_kursusles page-header">LEVEL DAN KEAHLIAN SAYA</h3>
            <div id="data_content">
				...
				<div class="boxSquare">
                    <div class="segmenKeahlian">
                        <div class="panel panel-default">
                            <div class="panel-heading">Keahlian Saya</div>
                                    ...
                                    <div class="tab-content">
                                        <?=$tabcontent?>
                                         ...
                                         <form method='POST' name='frm' id='frm' action=''>
                                         	<input type=hidden name=id_pengajar id=id_pengajar value='<?=$idPengajar?>'>
                                         	
                                         	<table>
                                         		<tr>
                                         			<td>Nama Mata Pelajaran</td>
                                         			<td>:</td>
                                         			<td><input type=text name=nama_mapel id=nama_mapel value='<?=$xx?>'></td>
                                         		</tr>
                                         		<tr>
                                         			...
                                         			<td>
                                         				<select name='jenjang_pendidikan'>
                                         					...
                                         					foreach($jenjang as $jnj){
                                         						echo "<option value='$jnj'>$jnj</option>";
                                         					}
                                         					?>
                                         				</select>
                                         			</td>
                                         ...
                                         <input type=button name='addnew' id='addnew' value='  Add New  ' onClick="document.getElementById('frm').submit()">
                                         </form>
                                    </div>
                                </div>
                            </div>
