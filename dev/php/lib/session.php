...
class Session{
	...
	function registerSessionGlobal($iduserPrime, $sNAMA){

		$db			=	new Db_connection();
		$enkripsi	=	new Enkripsi();

		//INSERT LOG ACTIVITY
		$sqlinsert	=	sprintf("INSERT INTO log_login (IDUSER, DATETIME)
								 SELECT %s AS IDUSER, NOW() AS DATETIME"
								, $iduserPrime
								);
		$db->execSQL($sqlinsert, 0);
	...
	function getTemplate($type = 'header', $login = 'false', $param_t1 = '', $param_t2 = '', $username = '', $password = ''){
		...
		if($login == 'true'){
			...
			$top_header	=	str_replace(
									...
									array(APP_IMG_URL, $enkripsi->encode($_SESSION['KursusLes']['FOTO']), $_SESSION['KursusLes']['NAMA'], $show_veri, $enkripsi->encode($_SESSION['KursusLes']['IDPRIME']), $indexuser),
									$top_header
								   );
		} else {
