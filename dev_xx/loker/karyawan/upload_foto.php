<?php
	...
	session_start();
	$sql		=	sprintf("SELECT FOTO FROM m_karyawan WHERE IDKARYAWAN = %s", $idkary);
	$result		=	$db->query($sql);
	$foto		=	$result[0]['FOTO'];
	...
	if( $enkripsi->decode($_GET['func']) == "setFoto" && isset($_GET['func'])){
		...
		$filename	=	$enkripsi->encode($idkary).'.jpg';
		
		//RENAME FILE LAMA JIKA ADA
		if(file_exists('../../dev/images/karyawan/'.$filename)){
			rename('../../dev/images/karyawan/'.$filename, '../../dev/images/karyawan/rn_'.date('YmdHis').$filename);
		}
		
		//RENAME TEMPORARY FILE
		if(file_exists('../../dev/images/karyawan/tmp_'.$filename)){
			
			$sqlUpdate	=	sprintf("UPDATE m_karyawan
									 SET FOTO			=	'%s'
									 WHERE IDKARYAWAN	=	%s"
									, $filename
									, $idkary
			...
			if(rename('../../dev/images/karyawan/tmp_'.$filename, '../../dev/images/karyawan/'.$filename)){
...
<head>
	...
	<?php
	if($foto <> "" && $foto <> 'null'){
		$style_foto		=	"background: url('".APP_IMG_URL."generate_pic.php?type=kr&q=".$enkripsi->encode($foto)."') no-repeat scroll -8px top / cover transparent;";
	}
	?>
	</style>
</head>
<body>
	...
	<script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141213'.'jssfileapiexif');?>.jsfile"></script>
	<script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141213'.'jssfileapi');?>.jsfile"></script>
	<script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141213'.'jssjqueryfileapi');?>.jsfile"></script>
	<script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141213'.'jssjqueryjcrop');?>.jsfile"></script>
	<script src="<?=APP_URL?>php/lib/file_request.php?get=<?=$enkripsi->encode('20141213'.'jssjquerymodal');?>.jsfile"></script>
