...
$sql	=	sprintf("SELECT IDLEVEL FROM m_user WHERE IDUSER = %s", $iduser);
$result	=	$db->query($sql);

//JIKA DATA KIRIMAN TIDAK KOSONG	
if($iduser <> ''){

	//JIKA ADA KIRIMAN RESET
	if($enkripsi->decode($_GET['t']) == "resetPwd" && isset($_GET['t'])){
		
		$sql		=	sprintf("UPDATE m_user SET PASSWORD = '%s' WHERE IDUSER = %s", $password, $iduser);
		...
		if($affected){
			...
			$redir	=	"<script>window.location.href = 'auth?u=".$_GET['u']."&p=".$password."&login_type=".$idlevel."';</script>";
		...
		$sql		=	sprintf("UPDATE m_user SET STATUS = 1 WHERE IDUSER = %s AND PASSWORD = '%s'", $iduser, $password);
		$affected	=	$db->execSQL($sql, 0);
